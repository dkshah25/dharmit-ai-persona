...
This document outlines the failure modes identified during the testing and development of the Scaler AI Persona Platform, explaining the root causes, business/technical impact, and the corresponding engineering fixes implemented to resolve them.
...
### Fix Implemented
1. **Rule-Based Input Guardrails**: Developed [guardrails.py](file:///d:/scalar%20ai%20screening/backend/guardrails.py) which contains regex filters matching common jailbreak templates (e.g. `ignore instructions`, `system prompt`, `dan mode`, `reveal instructions`, `you are now a`).
...
3. **Structured System Prompt**: Designed the system prompt in [main.py](file:///d:/scalar%20ai%20screening/backend/main.py) with distinct `--- START RETRIEVED CONTEXT ---` brackets to ensure the model distinguishes developer rules from user inputs.
...
### Fix Implemented
1. **Strict ISO ISO 8601 formatting**: Enforced that all start times handled by [scheduler.py](file:///d:/scalar%20ai%20screening/backend/scheduler.py) are ISO 8601 compliant, with explicit offsets (e.g., ending in `Z` for UTC or `-05:00` for EST).
...
### Fix Implemented
1. **Cosine Space Optimization**: Configured the ChromaDB collection to use `cosine` space in [rag.py](file:///d:/scalar%20ai%20screening/backend/rag.py). In cosine space, distances are normalized: $d = 1.0 - \text{cosine\_similarity}$.
2. **Confidence Computation**: We calculate the confidence score as: $\text{confidence} = 1.0 - d$.
3. **Strict RAG System Instructions**: In [main.py](file:///d:/scalar%20ai%20screening/backend/main.py), we instructed the LLM:
...
### Fix Implemented
1. **Multi-Model Failover Rotation**: Rewrote `safe_chat_completion` in [main.py](file:///d:/scalar%20ai%20screening/backend/main.py) to implement instant failover. If the primary model (`llama-3.1-8b-instant`) returns a 429 error, the backend immediately rotates to `gemma2-9b-it`, and then to `mixtral-8x7b-32768` in sequence with **no wait time**.
